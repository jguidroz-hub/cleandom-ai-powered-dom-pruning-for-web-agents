...
export default function ConfigsPage() {
  const [items, setItems] = useState<ConfigsItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [showCreate, setShowCreate] = useState(false);
  const [newTitle, setNewTitle] = useState('');

  useEffect(() => {
    fetch('/api/extractionConfigs')
      .then(r => r.json())
  ...
  const handleCreate = async () => {
    ...
    const res = await fetch('/api/extractionConfigs', {
      method: 'POST',
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/extractionConfigs/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Extraction Configurations</h1>
        <button
      ...
      <p className="text-gray-600 mb-6">Manage custom pruning rules</p>
